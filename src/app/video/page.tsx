...
const Page = () => {
    ...
    if (!userId || !roomId) {
      ...
      axios
        ...
        .then((result: any) => {
          ...
          enumDevices();
          console.log("function ll");
          createRoom(userId, userId, roomId, result?.data?.data);
          // startPublishingStream(streamIdState);
        })
        .catch((error: any) => {
  ...
  //Get the camera and mic devices
  const enumDevices = async () => {
    ...
    audioDeviceList.push({ deviceID: "0", deviceName: "None" });
    ...
    videoDeviceList.push({ deviceID: "0", deviceName: "None" });
  ...
  zg.on("roomUserUpdate", (roomID, updateType, userList) => {
    ...
    if (updateType == "ADD") {
      for (var i = 0; i < userList.length; i++) {
        ...
        setRemoteStreamIdState(userList[i]["userID"]);
      }
  ...
  const createRoom = (
    ...
    console.log("function room id: " + roomID + " userId: " + userID);
    zg.loginRoom(
      roomID,
      token,
      ...
      .then(async (result) => {
        if (result == true) {
          ...
          console.log("function streamId L: " + streamIdState);
          startPublishingStream(streamIdState);
        }
      })
      .catch((error) => {
  ...
  const startPublishingStream = async (streamId: any) => {
    try {
      const localStream = await zg.createZegoStream({
        camera: {
          video: true //cameraCheckStatus
            ? {
                input: cameraDevicesVal,
              }
            : false,
          audio: true //microphoneCheckStatus
            ? {
  ...
  const startPlayingStream = async (streamId: any, options = {}) => {
    try {
      console.log("function remote 112 : " + streamId);

      const remoteStream = await zg.startPlayingStream(streamId);
      console.log("function remote 112 : " + remoteStream);
      setRemoteStreamState(remoteStream);

      const remoteView = zg.createRemoteStreamView(
  ...
  const handleRoomStreamUpdate = async (
    ...
    if (updateType == "ADD") {
      ...
      const userStreamID = streamList[0].streamID;
      // The stream list specified by `streamList` contains the ID of the corresponding stream.
      // const remoteStream = await zg.startPlayingStream(userStreamID);
      startPlayingStream(userStreamID);
  ...
  const setProfilePicture = () => {
    ...
    zg.setDummyCaptureImagePath(vercel, localStreamState)
      .then((result) => {
        console.log({ message: "fucntion image setted successfully", result });
      })
      .catch((error) => {
        console.log({ message: "fucntion image setted failed", error });
      });
    console.log("function streamId L: " + streamIdState);
    // startPublishingStream(streamIdState);
  };

  // Check if the camera of the other user is closed (mute) or open
  const handleRemoteCameraStatusUpdate = (
    streamID: string,
    status: "OPEN" | "MUTE"
  ) => {
    console.log("function camera status: " + status);
    if (status === "OPEN") {
      console.log("function camera status 1: " + status);
    } else {
      console.log("function camera status 2: " + status);
    }
  };

  useEffect(() => {
    zg.on("remoteCameraStatusUpdate", handleRemoteCameraStatusUpdate);
  });

  const clearData = () => {
    localStreamState && zg.destroyStream(localStreamState);
    remoteStreamState && zg.destroyStream(remoteStreamState);

    setLocalStreamState(null);
    setRemoteStreamState(undefined);
  };
  ...
  const handleDecline = () => {
    ...
    console.log({ message: "function remote 1 : ", value: remoteStreamState });
    zg.stopPublishingStream(streamIdState?.toString() || "");
    remoteStreamIdState && zg.stopPlayingStream(remoteStreamIdState);

    clearData();
  };

  const handleMuteMic = () => {
    const mutel = zg.muteMicrophone(!muteMic);
    const muteP = zg.mutePublishStreamAudio(localStreamState, !muteMic);
    console.log("function mutel: " + mutel + " muteP: " + muteP);
    setMuteMic(!muteMic);
  };
  ...
  const handleMuteCam = () => {
    // if (!muteCam) {
    // zg.stopPublishingStream(streamIdState?.toString() || "");
    const muteP = zg.mutePublishStreamVideo(localStreamState, !muteCam);
    // localStreamState && zg.destroyStream(localStreamState);
    console.log("function mutel: " + "none" + " muteP: " + muteP);
  ...
  return (
    <div className="p-5 w-full bg-green-600 h-screen">
      ...
      <div className="w-full h-[10%] bg-white flex items-center justify-center gap-5">
        ...
        <button className=" px-5 py-2 rounded-3xl" onClick={handleMuteMic}>
          {muteMic ? (
        ...
        <button className=" px-5 py-2 rounded-3xl" onClick={handleMuteCam}>
          {muteCam ? (
            <BsCameraVideoOff color={"#000000"} size={40} />
          ) : (
            <BsCameraVideo color={"#000000"} size={40} />
          )}
        </button>
      </div>
    </div>
